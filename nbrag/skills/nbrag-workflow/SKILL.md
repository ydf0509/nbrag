...
**核心原则**：你（AI）是最好的 query rewriter。不要把用户的长问题原封不动丢给 `nbrag_search`，而是主动拆解、改写、组合多次检索。

**示例**：用户问 "公司让我试用期干了5个月不转正，签的1年合同，合法吗？"
- 第1轮: `nbrag_search("试用期 最长期限 1年合同")` → 找到试用期上限规定
- 第2轮: `nbrag_search("违法约定试用期 赔偿")` → 找到赔偿标准
- 第3轮: `nbrag_grep("第十九条")` → 精确定位法条原文

如果第一轮搜索结果不理想：

1. **重写查询**：将用户口语化问题改写为更精确的检索词（如 "冷静期" → "三十日内 撤回"）
