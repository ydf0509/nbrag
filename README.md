...
| **混合检索** | `nbrag_search_and_fetch` | 混合搜索 + 自动取源码（省一次 round-trip） |
...
```
1. nbrag_search → 混合检索（语义 + BM25 关键词匹配），找到相关文件和大致位置
2. nbrag_grep → 精确定位类名/方法名/常量（行级精度补充）
...
分词器针对代码做了特殊处理：拆 camelCase (`getUserById` → `get user by id`)、拆 snake_case、去 chunk header。
